...
export const recoverDocumentFromCloud = async (
  ...
  try {
    ...
    const document = await convex.query(api.documents.getDocumentForRecovery, { documentId })
...
export const createFallbackDocument = (): Y.Doc => {
  ...
  newDoc.transact(() => {
    ...
    sharedType.insert(sharedType.length, 'This document experienced a local storage issue. ')
    sharedType.insert(sharedType.length, 'Your content is safely stored in the cloud and will be restored automatically.')
  })
...
export const useCloudRecovery = () => {
  ...
  const recoverDocument = async (documentId: Id<"documents">): Promise<Y.Doc | null> => {
