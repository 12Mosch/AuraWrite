 ...
 */
const categorizeConvexError = (error: Error): {
  ...
  if (errorMessage.includes('read/write limit') || errorMessage.includes('too much data')) {
    return {
  ...
  if (errorMessage.includes('function not found') || errorMessage.includes('invalid arguments')) {
    return {
      category: ErrorCategory.VALIDATION,
      severity: ErrorSeverity.HIGH,
      recoveryStrategy: RecoveryStrategy.MANUAL,
...
export const ConvexErrorBoundary: React.FC<ConvexErrorBoundaryProps> = ({
  ...
  const convexFallback = fallback || ((error: Error, errorInfo: ErrorInfo, retry: () => void) => (
    <ConvexErrorFallback
      error={error}
      errorInfo={errorInfo}
      retry={retry}
