...
import type { SyncStats } from "../hooks/useOptimizedSync";
...
export const SyncPerformanceMonitor: React.FC<SyncPerformanceMonitorProps> = ({
	...
	const successRate =
		stats.totalUpdates > 0
			? (
					((stats.totalUpdates - stats.failedUpdates) / stats.totalUpdates) *
					100
				).toFixed(1)
			: "100.0";
		...
		<div className={`bg-gray-900 text-white text-xs font-mono ${className}`}>
			...
			>
				<div className="flex items-center space-x-4">
					...
					<div>{stats.averageLatency.toFixed(0)}ms</div>
...
export const CompactPerformanceIndicator: React.FC<
	CompactPerformanceIndicatorProps
> = ({ getStats, className = "" }) => {
	const [stats, setStats] = useState<SyncStats>({
		totalUpdates: 0,
		batchedUpdates: 0,
	...
	return (
		<div className={`flex items-center space-x-2 text-xs ${className}`}>
			<span role="img" aria-label={getStatusIcon().label}>
				{getStatusIcon().emoji}
			</span>
			<span className={getLatencyColor(stats.averageLatency)}>
				{stats.averageLatency.toFixed(0)}ms
			</span>
			<span className="text-gray-500">{stats.totalUpdates} updates</span>
		</div>
	);
};
