...
import { usePresence, getUserInitials, getUserColor, getLastSeenText } from '../hooks/usePresence';
...
const UserAvatar: React.FC<UserAvatarProps> = ({ 
  ...
  const avatar = (
    <div
      ...
      title={showTooltip ? `${user.name}${lastSeenText ? ` • ${lastSeenText}` : ''}` : undefined}
    >
      {user.image ? (
        <img
          src={user.image}
      ...
      {showNames && (
        <div className="flex flex-col text-sm text-gray-600">
          {visibleUsers.length === 1 ? (
            ...
            <span>{visibleUsers[0].user.name} and {visibleUsers[1].user.name}</span>
          ) : (
            <span>
              {visibleUsers[0].user.name} and {otherUsers.length - 1} other{otherUsers.length === 2 ? '' : 's'}
            </span>
          )}
        </div>
      )}
...
export const PresenceList: React.FC<PresenceListProps> = ({
  ...
  const { presence, isLoading, otherUsers, currentUser } = usePresence(documentId);
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      {allUsers.map((presenceUser) => (
        <div key={presenceUser.sessionId} className="flex items-center space-x-3">
          <UserAvatar
            user={presenceUser.user}
            size="md"
            showTooltip={false}
