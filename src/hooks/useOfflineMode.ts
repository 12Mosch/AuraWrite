...
export const useOfflineMode = (
	...
	const pendingOperations = offlineOperationsRef.current.filter(
		(op) => !op.applied,
	).length;
	...
	const addOfflineOperation = useCallback(
		(type: OfflineOperation["type"], data: any) => {
			...
			if (offlineOperationsRef.current.length > maxOfflineOperations) {
				offlineOperationsRef.current = offlineOperationsRef.current.slice(
			...
			try {
				...
				const storageError = ErrorFactory.persistence(
					...
					"localstorage",
					{
						...
						context: {
							documentId,
							operationCount: offlineOperationsRef.current.length,
						},
					},
				);
		...
		try {
			...
			if (stored) {
				...
				setHasUnsyncedChanges(
					offlineOperationsRef.current.some((op) => !op.applied),
				);
			}
		} catch (error) {
	 ...
	 */
	const applyOfflineOperation = useCallback(
		async (operation: OfflineOperation): Promise<void> => {
			...
			try {
				switch (operation.type) {
					...
					case "format":
						...
						if (
							...
							console.warn(
								"Format operation missing position, length, or attributes data",
							);
						}
						break;
				...
				console.log(
					`Successfully applied offline operation: ${operation.type} (${operation.id})`,
				);
			} catch (error) {
				console.error(
					`Failed to apply offline operation ${operation.id}:`,
					error,
				);
				throw new Error(
					`Failed to apply offline operation: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		},
		[yDoc],
	...
	const syncOfflineOperations = useCallback(async (): Promise<boolean> => {
		...
		try {
			...
			offlineOperationsRef.current = offlineOperationsRef.current.filter(
				(op) => !op.applied,
			);
	 ...
	 * Resolve conflicts manually
	 */
	const resolveConflictsManually = useCallback(
		async (resolution: "local" | "remote" | "merge"): Promise<void> => {
...
async function checkForConflicts(
	...
	try {
		...
		if (
			currentState.length === serverState.length &&
		...
		const localDiff = Y.diffUpdate(
			Y.encodeStateAsUpdate(localDoc),
			serverStateVector,
		);
		const serverDiff = Y.diffUpdate(
		...
		if (hasConflict) {
			console.log(
				"Conflict detected: both local and server have divergent changes",
			);
		}
