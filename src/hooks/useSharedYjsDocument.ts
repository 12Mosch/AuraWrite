...
class YjsDocumentManager {
  ...
  getDocument(documentId: string, options: {
    ...
    if (!this.documents.has(documentId)) {
      ...
      if (options.enablePersistence) {
        ...
        if (docInfo.sharedType.length === 0 && options.initialValue && options.initialValue.length > 0) {
          console.log('Loading initial value into shared Y.Doc (no persistence)')
          docInfo.yDoc.transact(() => {
            docInfo.sharedType.applyDelta(slateNodesToInsertDelta(options.initialValue!))
          })
        }
  ...
  private async setupPersistence(documentId: string, docInfo: any, options: any) {
    try {
      const indexeddbProvider = new IndexeddbPersistence(documentId, docInfo.yDoc)
      docInfo.indexeddbProvider = indexeddbProvider

      // Handle sync events
      indexeddbProvider.whenSynced.then(() => {
        console.log(`Shared Y.Doc synced with IndexedDB for document: ${documentId}`)
        docInfo.isSynced = true
        ...
        if (error.message?.includes('corrupt') || error.message?.includes('invalid')) {
          console.log('Attempting to recover corrupted shared document from cloud...')
          try {
            if (options.recoverDocument) {
              const recoveredDoc = await options.recoverDocument(documentId as Id<"documents">)
              if (recoveredDoc) {
                ...
                docInfo.persistenceError = 'Document was recovered from cloud. Some recent changes may have been lost.'
                ...
                console.log('Using fallback document with recovery message for shared doc')
                docInfo.persistenceError = 'Document recovery failed. Started with recovery message.'
                fallbackDoc.destroy() // Clean up fallback document
              }
            }
          } catch (recoveryError) {
            console.error('Cloud recovery failed for shared document:', recoveryError)
            docInfo.persistenceError = 'Document recovery failed. Starting with empty document.'
          }
        } else {
      ...
      indexeddbProvider.on('error', (error: Error) => {
        ...
        if (error.name === 'QuotaExceededError') {
          docInfo.persistenceError = 'Local storage quota exceeded. Please free up space or clear old documents.'
        } else if (error.name === 'InvalidStateError') {
          docInfo.persistenceError = 'Local storage is unavailable. Changes will not be saved locally.'
        } else {
      ...
      console.error('Failed to initialize IndexedDB persistence for shared document:', error)
      docInfo.persistenceError = 'Failed to initialize local storage. Changes will not be saved locally.'
      docInfo.persistenceAvailable = false
    }
  }
 ...
 */
export const useSharedYjsDocument = (options: UseSharedYjsDocumentOptions): UseSharedYjsDocumentReturn => {
  const {
    documentId,
  ...
  }, [documentId, enablePersistence, enableGarbageCollection, recoverDocument, createFallback, initialValue])
  ...
  useEffect(() => {
    return () => {
      const docIdString = typeof documentId === 'string' ? documentId : String(documentId)
      console.log(`Releasing shared Y.Doc reference for document: ${docIdString}`)
