...
import { ErrorFactory, ErrorSeverity, type SyncError } from "../types/errors";
 ...
 */
function detectMetadataConflicts(
	localDoc: Y.Doc,
	remoteDoc: Y.Doc,
): Array<ConflictInfo> {
	const conflicts: Array<ConflictInfo> = [];
		...
		for (const field of criticalFields) {
			...
			if (
				...
				conflicts.push({
					...
					remoteValue,
					resolution:
						field === "lastModified" ? "use_latest" : "manual_required",
				});
			}
		}
	} catch (error) {
