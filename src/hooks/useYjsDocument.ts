...
export const useYjsDocument = (
		...
		if (enablePersistence) {
			try {
				...
				indexeddbProvider.whenSynced
					.then(() => {
						console.log(
							`Y.Doc synced with IndexedDB for document: ${documentId}`,
						);
					...
					.catch(async (error) => {
						...
						if (
							...
							console.log(
								"Attempting to recover corrupted document from cloud...",
							);
							try {
								const recoveredDoc = await recoverDocument(
							...
							setPersistenceError(
								`Failed to sync with local storage: ${error.message}`,
							);
						}
	...
	useEffect(() => {
		...
		return () => {
			console.log(
				`Cleaning up Y.Doc event listeners for document: ${documentId}`,
			);
